"""Test WARNING SIGN detection on data/warning signs/*.jpg."""
import glob
import os
import sys
...
def detect_warning(mask, frame):
    ...
    for c in contours:
        ...
        if v in (4, 5):
            detected += 1
            draw_label(frame, bb, "WARNING SIGN", WARNING_COLOR)
    return detected
...
def main():
    ...
    for path in paths:
        ...
        det = detect_warning(get_static_yellow_mask(frame), frame)
